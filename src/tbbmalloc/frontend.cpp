        ...
        public:
            ...
            void checkFreePrecond(const void *object) const
            {
                ...
                else
                {
                    ...
                    MALLOC_ASSERT(allocatedCount <= (slabSize - sizeof(Block)) / objectSize && (!bumpPtr || object > bumpPtr), msg);
...
#error highestBitPos() not implemented for this platform
        ...
        unsigned int getSmallObjectIndex</*Is32Bit=*/false>(unsigned int size)
        {
            ...
            if (result)
                result |= 1; // 0,1,3,5,7; bins 2,4,6 are not aligned to 16 bytes
            return result;
        }
        /*
        ...
        static unsigned int getIndexOrObjectSize(unsigned int size)
        {
            ...
            { // selection from 8/16/24/32/40/48/56/64
            ...
            {                                                 // 80/96/112/128 / 160/192/224/256 / 320/384/448/512 / 640/768/896/1024
        ...
        Block *MemoryPool::getEmptyBlock(size_t size)
        {
            ...
            FreeBlockPool::ResOfGet resOfGet = tls ? tls->freeSlabBlocks.getBlock() : FreeBlockPool::ResOfGet(NULL, false);
            Block *result = resOfGet.block;

            if (!result)
            { // not found in local cache, asks backend for slabs
        ...
        Block *Bin::getPrivatizedFreeListBlock()
        {
            ...
            { // mailbox is not empty, take lock and inspect it
        ...
        void Block::privatizePublicFreeList(bool reset)
        {
            ...
            if (isSolidPtr(localPublicFreeList))
            {
                ...
                { // the list will end with either NULL or UNUSABLE
                    temp = temp->next;
                    allocatedCount--;
                    MALLOC_ASSERT(allocatedCount < (slabSize - sizeof(Block)) / objectSize, ASSERT_TEXT);
                }
        ...
        void StartupBlock::free(void *ptr)
        {
            Block *blockToRelease = NULL;
            {
                ...
                MALLOC_ASSERT((uintptr_t)ptr >= (uintptr_t)this + sizeof(StartupBlock) && (uintptr_t)ptr + StartupBlock::msize(ptr) <= (uintptr_t)this + slabSize,
                              ASSERT_TEXT);
                if (0 == --allocatedCount)
                {
        ...
        void *MemoryPool::getFromLLOCache(TLSData *tls, size_t size, size_t alignment)
        {
            ...
            if (lmb)
            {
                ...
                { // !tls is cold path
                    ...
                    unsigned numOfPossibleOffsets = alignment == estimatedCacheLineSize ? ptrDelta / estimatedCacheLineSize : ptrDelta / alignment;
        ...
        static void *allocateAligned(MemoryPool *memPool, size_t size, size_t alignment)
        {
            ...
            LargeObjAlloc:
                ...
                result =
                    memPool->getFromLLOCache(tls, size, largeObjectAlignment > alignment ? largeObjectAlignment : alignment);
            }
                                    ...
                                    size_t newSize, size_t alignment = 0)
        {
            ...
            if (isLargeObject<ourMem>(ptr))
            {
                ...
                if (void *r = memPool->extMemPool.remap(ptr, copySize, newSize,
                    ...
                    return r;
#endif
                result = alignment ? allocateAligned(memPool, newSize, alignment) : internalPoolMalloc(memPool, newSize);
            }
            else
            {
        ...
        bool isLargeObject(void *object)
        {
            if (!isAligned(object, largeObjectAlignment))
                return false;
            LargeObjectHdr *header = (LargeObjectHdr *)object - 1;
            BackRefIdx idx = (memOrigin == unknownMem) ? safer_dereference(&header->backRefIdx) : header->backRefIdx;
        ...
        static inline void freeSmallObject(void *object)
        {
            ...
            { /* Slower path to add to the shared list, the allocatedCount is updated by the owner thread in malloc. */
